...
router.post('/', async (req, res) => {
  const { employee_id, trx_id, sertif_name, certificate_number, issue_date, expiry_date, issuing_body, card_number, card_date, competency_id, delivery_method, certification_type, is_lifetime, is_active, renewal_count, notes, renewal_action, renewal_cert_id } = req.body;
  ...
  await logAudit({
    ...
    operation: 'CREATE',
    new_data: { employee_id, trx_id, sertif_name, certificate_number, issue_date, expiry_date, issuing_body, card_number, card_date, competency_id, delivery_method, certification_type, is_lifetime, is_active, renewal_count, notes, renewal_action, renewal_cert_id },
...
router.put('/:id', async (req, res) => {
  ...
  // Pertahankan nilai renewal_count dari DB jika tidak dikirim dari frontend
  const newRenewalCount = renewal_count != null ? renewal_count : old[0].renewal_count;
  ...
  await logAudit({
    ...
    operation: 'UPDATE',
    old_data: old[0],
    new_data: { employee_id, trx_id, sertif_name, certificate_number, issue_date, expiry_date, issuing_body, card_number, card_date, competency_id, delivery_method, certification_type, is_lifetime, is_active, renewal_count: newRenewalCount, notes, renewal_action, renewal_cert_id },
    changed_by: req.changedBy,
  });

  const [result] = await pool.query(
    `UPDATE trx_certification
     SET employee_id=?, trx_id=?, sertif_name=?, certificate_number=?, issue_date=?, expiry_date=?, issuing_body=?, card_number=?, card_date=?, competency_id=?, delivery_method=?, certification_type=?, is_lifetime=?, is_active=?, renewal_count=?, notes=?, renewal_action=?, renewal_cert_id=?
     WHERE cert_id=?`,
    [employee_id, trx_id || null, sertif_name, certificate_number || null, issue_date, is_lifetime ? null : (expiry_date || null), issuing_body || null, card_number || null, card_date || null, competency_id || null, delivery_method || 'Offline', certification_type || 'Eksternal', is_lifetime ?? 0, is_active ?? 1, newRenewalCount, notes || null, renewal_action || null, renewal_cert_id || null, req.params.id]
  );
